...
// {
// 	ansbit.reset();
// 	for (int i = 0;i < n * 3;i++) if (ans[i]) ansbit.set(i);
// 	ansbit.set(n*3);
// 	for (int x = 0; x < n; x++) {
// 		// if (x % 100 == 0) fprintf(stderr, "reconstruct: %d\n", x);
// 		for (int y = 0;y < n;y++) {
// 			int tans = (expr[x][y] & ansbit).count() & 1;
// 			finalans[x][y] = tans;
// 		}
// 	}
// 	return 0;
...
int main(void)
{
	...
	// 		if (expr[x][y].test(freevars[i])) {
	// 			affected[i].push_back(make_pair(x, y));
	// 			affectedSize++;
	// 		}
	// 	}
	// }
	// printf("Affected size: %d\n", affectedSize);
	...
	// int minans = ~0U>>1;
	// int ans = 0;
	// for (int i = 0;i < n;i++) for(int j = 0;j < n;j++) ans += finalans[i][j];
	// int fvl = freevars.size();
	// minans = ans;
	// for(int msk = 0;msk < (1<<fvl);msk++) {
	// 	int delta = 0;
	// 	for (int i = 0;i < fvl;i++) 
	// 		if (((msk >> i) & 1) != finalans[freevars[i]/n][freevars[i]%n]) {
	// 			if (finalans[freevars[i]/n][freevars[i]%n]) {
	// 				finalans[freevars[i]/n][freevars[i]%n] = 0;
	// 				delta--;
	// 			} else {
	// 				finalans[freevars[i]/n][freevars[i]%n] = 1;
	// 				delta++;
	// 			}
	// 			for (const auto& x : affected[i]) {
	// 				// printf("affects: %d %d\n", x.first, x.second);
	// 				if (finalans[x.first][x.second]) delta--;
	// 				else delta++;
	// 				finalans[x.first][x.second] ^= 1;
	// 			}
	// 		}
	// 	ans += delta;
	// 	//if (ans < minans) {
